...
> **TL;DR.** A skill enforces 10 flat steps (0–9) organized around **two gates** — Verify (Step 5, "does it work?") and Review (Step 6, "is it well-made?") — plus a cross-cutting **commit rhythm** that fires per step rather than at a numbered position. **v12 (current)** declares an orthogonal **`intent · rigor · scale` triple** per run: intent ∈ `build`/`bugfix`/`refactor`/`tune`/`spike`/`incident-response`; rigor ∈ `tested`/`peer-reviewed`/`audited`; scale ∈ `task`/`wave` (default)/`epic`/`continuous` — three BOUNDED rungs plus one STANDING rung, the `continuous` charter (v11 froze scale at three values; v12 widens it to four and adds symmetric fail-loud version enforcement — see [Versioning](#versioning-and-backward-compat)). `mode:` is **v≤10 legacy vocabulary**, grandfathered forever and never retrofitted. Plan frontmatter is the single source of truth: `governing-skill`, the `intent`/`rigor`/`scale` triple, `sdlc-step`, `canonical_sdlc_version`, five discriminator flags, two opt-in flags, and a `model_plan` line. State lives in a `## SDLC State` section, one evidence line per step (or per ledger task at `scale: task`, or per goal at `scale: continuous`). Four hooks back the skill: `governing-skill` validates plan/spec/ADR frontmatter on write (v12: triple presence + whole-value enums, a `mode:` line blocked, barred intent×scale cells blocked, `continuous`'s audited scale floor hard-blocked); `evidence-gate` blocks commits whose current-step evidence line is missing or placeholder, and now fails loud on an unrecognized `canonical_sdlc_version`; `farm-out-reminder` mechanically enforces the dispatch split (tier-1 long-running main-thread commands are denied with a redirect naming the right agent role, tier-2 gets one nudge per class per session); and `context-spend` logs per-step context occupancy. The latter two share the skill's audit channel (`.bionic/memory/sdlc-v11-audit.md`). Model *tiering* — who runs on what — remains prose-governed.
...
**v11 replaced the single `mode:` with three orthogonal axes; v12 (current) widens the scale axis to a fourth value.** Every run declares exactly **one triple** — `<intent> · <rigor> · <scale>` — at Step 0. The axes are independent: any intent runs at any rigor (subject to floors) and any valid scale. **Intent** is the kind of work, **rigor** is how hard the evidence tries to lie, **scale** is the unit of commitment one run signs up for. Intent declaration is reviewable — a run mislabelled to dodge steps is drift with a label.
...
### Scale — the unit of commitment one run signs up for (one line each)

Three BOUNDED rungs, plus one STANDING rung with no fixed upper bound:
...
Default rigor is **scale-keyed** (provisional at Step 0, locked at Step 3): `audited` is the default at **wave scale and above**; the **task** lane keeps the lighter `tested`/`peer-reviewed` defaults (bugfix/spike → `tested`; build/refactor/tune → `peer-reviewed`; incident-response floors at `audited` everywhere); **`continuous`** (v12) only ever runs `build`/`refactor`/`tune` (the rest are barred, below) and floors at `audited` unconditionally. Effective rigor is the **MAX across five floor sources** — intent floor, flag floor (security/privacy/vulnerable-population → `audited`), project floor (`.bionic/config.yaml`), epic floor (epic frontmatter `rigor-floor:`), and the v12 **scale floor** (`scale: continuous` → `audited`) — a floor only pushes rigor **UP**, never down (upward-only). Unlike the first four (enforced as advisory, log-only consistency checks in the governing-skill hook), the scale floor is a **hard block** — a `continuous` plan declaring rigor below `audited` fails the write every time. Full mechanics: [SKILL.md §Rigor floors and lifecycle](SKILL.md).

### Barred intent × scale cells (D4)

`bugfix × epic`, `spike × epic`, and `incident-response × epic` are **barred** — a bugfix needing multi-session decomposition is a misclassified refactor/build; spikes and incidents are timeboxed/clock-driven, not epic-scoped. **v12 adds** `bugfix × continuous`, `spike × continuous`, and `incident-response × continuous` — a `continuous` charter's `intent:` is its default conversion stamp for every admitted goal (SR-1), and a repair/timebox/time-pressured response cannot serve as that default. The governing-skill hook blocks a barred triple on v11/v12 artifacts.
...
**v12 (current)** — declares the `intent`/`rigor`/`scale` triple; **no `mode:` line**:
...
**v12 charter (`scale: continuous`)** — same identity fields, `rigor: audited` (the scale floor), no `wave:` field:
...
**v11 (prior, still enforced)** — identical shape, frozen at three `scale:` values (`continuous` blocks):
...
A **v11/v12 artifact must NOT carry a `mode:` line** — the two vocabularies are mutually exclusive, and the governing-skill hook blocks a `mode:` line on a v11/v12 artifact as a split-brain guard (exit 2). Conversely, a v≤10 plan keeps `mode:` and never gains the triple. For incident-response artifacts, use `incident: NNNN-<slug>` instead of `epic`/`wave`; a `scale: continuous` charter omits `wave:` (it has none) and may keep `epic:` to name the scope it supervises.
...
| `intent` (v11/v12) | `build` \| `bugfix` \| `refactor` \| `tune` \| `spike` \| `incident-response` | The kind of work. Whole-value enum, hook-checked. Seeds governing-skill substitutions and default flags. At `scale: continuous`, this is the charter's default conversion stamp (SR-1) — `bugfix`/`spike`/`incident-response` are barred there. |
| `rigor` (v11/v12) | `tested` \| `peer-reviewed` \| `audited` | How hard the evidence tries to lie. Provisional at Step 0, locked at Step 3; floors push it up only. `scale: continuous` floors at `audited` unconditionally, hard-blocked. |
| `scale` (v11/v12) | v11: `task` \| `wave` \| `epic`. v12: adds `continuous`. | The unit of commitment. `task` uses a `## Tasks` ledger; `epic` short-circuits after Step 3; `continuous` (v12 only) is the standing charter scope, also short-circuiting after Step 3 but never terminal. Barred against some intents (§validity). The enum is version-scoped — `scale: continuous` on a v11 plan blocks. |
| `mode` (**v≤10 only**) | `autonomous` \| `epic-scope` \| `incident-response` \| `design-refresh` \| `spike` | **Legacy vocabulary — not written on v11/v12 plans.** Determined applicable steps, governing-skill substitutions, and default flag values under v≤10. See [The axis model (and legacy modes)](#the-axis-model-and-legacy-modes). |
| `canonical_sdlc_version` | `1`/`2` (legacy) \| `3` \| `4` \| `5` \| `6` \| `7` \| `8` \| `9` \| `10` \| `11` (prior) \| `12` (current) | Routes the hooks. See [Versioning](#versioning-and-backward-compat). |
| `epic` / `wave` / `incident` | Identifier matching the enclosing directory | `epic-NN-<slug>` / `wave-NN-<slug>` for epic-tree work; `NNNN-<slug>` for incidents. A `scale: continuous` charter carries `epic:` (if scoped to one) and no `wave:`. |

> **Universal structural contract (v11/v12, D13).** On v10 the 5 discriminator flags + 2 opt-in flags + `model_plan` + the `## Verification Matrix` (at `sdlc-step ≥ 3`) were required only on the `autonomous` mode. **v11 re-keys this off the triple's presence, not a mode value: the same structural contract applies to EVERY v11/v12 artifact regardless of intent** — a `bugfix`, a `refactor`, a `tune`, all carry the full flag set + `model_plan` + matrix. There is no `mode:` short-circuit to exempt a plan. **v12 adds one exemption**: `scale: continuous` charters carry a `## Goals` registry instead of a `## Verification Matrix` — exempt from the matrix requirement exactly as `scale: task` plans are.

### Discriminator flags (5 — required on v3–v10 autonomous plans **and all v11/v12 plans**)
...
On v11/v12, `## SDLC State` opens with `integration-branch:` and the `intent`/`rigor`/`scale` triple, then `current:` (a v≤10 plan opens with `mode:` here instead). `current: N` names the active step. When advancing, **replace `Step N: (pending)` in-place** — never append. (`Phase N:` is accepted as a legacy alias for `Step N:`.) At **`scale: task`** the section instead addresses a ledger task — `current: T<n>` plus one `- T<n>: <evidence>` line per task, backed by a `## Tasks` registration table (see [Task-scale ledger enforcement](#task-scale-ledger-enforcement-v11)). At **`scale: continuous`** (v12) the section instead addresses the charter's own scoping/arming lifecycle — `current: <0-3>` while scoping, `current: G<n>` once armed (addressing a `## Goals` registry row), `current: 10` at close (see [Charter contract (v12)](#charter-contract-v12)). The `## Assumptions` section sits alongside it, seeded from Step 1's "Not Doing" list plus spec ambiguities, and appended inline during Step 4.
...
|---|---|---|
| **Verification** | Yes — mandatory | `canonical-sdlc-evidence-gate.sh` (presence + per-step shape on v3–v12; v11/v12 wave/epic share the v10 shape; v12 `scale: continuous` runs the charter contract instead) |
...
### Per-step evidence shape — the v10 table (shared by v11/v12 wave/epic)

**v11 and v12 wave- and epic-scale plans run the v10 per-step table below unchanged** — same per-step fields, same `## Verification Matrix` validation (a unified arm: `gate =11||=12` in the evidence-gate). v11/v12 re-key the *gate* off the triple, not the evidence shape; the per-step shapes are identical. `scale: task` plans instead run the [task-scale ledger enforcement](#task-scale-ledger-enforcement-v11) below — now **blocking**, rigor-keyed, tightened in place under `canonical_sdlc_version: 11` and inherited verbatim by `12`. Audited multi-agent waves additionally carry [D7 dispatched-task ledger presence](#d7-dispatched-task-ledger-presence-wave-scale), and [intent-conditional Step-5 keys](#intent-conditional-step-5-keys-v11) round out the v11 additions, likewise inherited by v12. **v12 `scale: continuous` plans instead run the [charter contract](#charter-contract-v12)** — a new evidence shape, not an extension of the table below.
...
#### Charter contract (v12)

A `scale: continuous` plan (the charter) is a **first-class plan kind, not a bigger epic** — heterogeneity distinguishes it: an epic decomposes homogeneously into waves of one intent, while a charter supervises a heterogeneous stream of task/wave/epic sub-goal runs, each carrying its own triple. Its `intent:` is the default conversion stamp every admitted goal gets (SR-1); `rigor:` floors at `audited` unconditionally (§Scale-keyed rigor defaults and floors), hard-blocked. Charter-scoping runs Steps 0–3 exactly like epic-scoping, producing `continuous.plan.md` at `<docs-root>/plans/<scope>/continuous.plan.md`; its `## Verification Matrix` requirement is exempt (it carries a `## Goals` registry instead, matrix-exempt on the same footing as `scale: task`).

The plan body carries `## Charter` (conversion policy, gate policy, prioritization, completion condition — e.g. drain-then-close: no queued/active goals AND an empty backlog), `## Goals` (the registry), `## SDLC State` (the charter's OWN scoping/arming lifecycle), and `## Assumptions`. The `## Goals` registry is a 4-column table — `| id | triple | plan | status |`, `id` matching `^G[0-9]+$`, `status ∈ queued|active|complete|gated|dropped` — pointing at each goal's own plan file. **Boundedness (A5):** the registry holds goal ROWS (pointers) only; backlog intake (an inbox file, a status file) lives OUTSIDE the plan — the hook never counts rows, only validates the addressed one and every non-queued one.

`## SDLC State` addresses three forms: `current: <0-3>` while charter-scoping (epic-style presence rules), `current: G<n>` once armed, `current: 10` at close. The evidence-gate hook's charter contract validates `current: G<n>`: the addressed `## Goals` row must exist, its status must be a valid enum (blocks on ANY row using an invalid one), and it — plus every OTHER non-queued row — must carry a non-placeholder `- G<n>: <evidence>` line inside `## SDLC State` (queued rows are exempt). A malformed row (wrong cell count — a literal `|` inside a cell shears it, mirroring the matrix grammar) blocks. **`current: G<n>` is valid ONLY on a `scale: continuous` plan** — on any other v12 scale it blocks naming the mismatch; on a v≤11 plan it is not a recognized form and falls through to the numeric-`current:` parse failure, unchanged (never retrofitted).
...
- **v12 plans (current)** — the triple's presence is the gate, with **no `mode:` short-circuit (D13)**. On any v11/v12 artifact the hook **blocks (exit 2)** on: a `mode:` line (split-brain guard), a missing or non-enum (non-whole-value) `intent:`/`rigor:`/`scale:`, a **barred** intent × scale cell (§validity), a missing discriminator/opt-in flag or `model_plan`, and — on `*.plan.md` at `sdlc-step ≥ 3`, exempting `scale: task` and (v12 only) `scale: continuous` — a missing `## Verification Matrix` section. This **universal structural contract** applies regardless of intent (no `autonomous`-only gating). The scale enum is **version-scoped**: v12 accepts `task`\|`wave`\|`epic`\|`continuous`; v11 stays frozen at three — `scale: continuous` on a v11 plan blocks. v12 adds `bugfix`\|`spike`\|`incident-response` × `continuous` to the barred cells (charter default-conversion stamp, SR-1), and hard-blocks `scale: continuous` with `rigor:` below `audited` — the fifth floor source, and the only one enforced as a hard block rather than log-only (§Scale-keyed rigor defaults and floors). The hook additionally runs **log-only floor-consistency checks** (intent floor, spike cap, project floor, epic floor → one line to `.bionic/memory/sdlc-v11-audit.md` + stderr, exit 0 — never blocks).
- **v11 plans (prior, still enforced)** — identical contract, frozen at three `scale:` values.
...
**Fail-loud version dispatch (v12, AC-8).** A version-allowlist gate now fronts the dispatch chain below: `canonical_sdlc_version` `1`–`12` (and an absent version line — the pre-versioning legacy case) pass through; any other non-empty value **blocks (exit 2)**, naming the supported set — replacing the previous silent `exit 0` fall-through that gave an unrecognized version (`13`, garbage, or the formerly-unarmed `12`) zero evidence gating. This is symmetric with the governing-skill hook's pre-existing unknown-version block.

Shape enforcement is version-routed:
- **v12 plans (current)** — wave/epic/task plans inherit the v11 contract below **verbatim** (a unified arm: `gate =11||=12`). `scale: continuous` charters instead run the **charter contract**: see [Charter contract (v12)](#charter-contract-v12).
- **v11 plans (prior, still enforced)** — wave/epic plans carry the **v10 shape table and `## Verification Matrix` validation unchanged** (v11 re-keys the gate off the triple, not the evidence shape). `scale: task` plans instead run the **rigor-keyed ledger contract**: the hook **accepts `current: T<n>`** without blocking (a false block there is a defect), then **blocks** on the addressed unit's tested-floor presence check and its own effective-rigor lanes — proof-shape (a digit + a command token) at `peer-reviewed`/`audited`, an `auditor` token on `done` rows at `peer-reviewed`/`audited`, a `critic` token on `done` rows at `audited` — and an off-enum (unresolvable) rigor cell blocks on ANY row, at ANY status, unconditionally. The ledger-shape checks on non-addressed rows (missing `## Tasks`, bad status enum, an active/done row with no evidence) stay **log-only** unless frontmatter is `rigor: audited`, where they become **blocking** too. Separately, an `audited`, `multi_agent: true` plan at `scale: wave` must carry a `## Tasks` section at all — **D7 presence** — or the commit blocks; an empty section is fine, and any present rows validate at tested-floor shape only; `scale: epic` (and, on v12, `scale: continuous`) is excluded. What stays **log-only**: the **epic merge-target** check, and the **intent-scoped Step-5 keys** — check-id `refactor-evidence` (`behavior-preservation:` present; `compat-matrix:`/`revert-plan:` present-but-may-be-empty, with an `n/a: not a migration` escape) and check-id `tune-evidence` (`baseline:`/`target:`/`re-measure:` present) — each finding appends one line to `.bionic/memory/sdlc-v11-audit.md` + stderr and exits 0. Promotion of the remaining log-only checks to blocking is a later user decision from the audit data.
...
| `11` | Prior, still enforced | The `intent · rigor · scale` triple replaces `mode:`; the v10 structural contract (5 discriminators + 2 opt-in + `model_plan` + matrix at `sdlc-step ≥ 3`) becomes **universal** across every intent (D13) — no `mode:` short-circuit; the governing-skill hook blocks a `mode:` line, a non-enum triple, and barred intent × scale cells. Scale is **frozen at three values** — `scale: continuous` on a v11 plan blocks | v10 shape table + matrix **unchanged** on wave/epic plans; adds `scale: task` ledger addressing (`current: T<n>`), **blocking and rigor-keyed** — three cumulative lanes on the addressed row (tested floor; peer-reviewed proof-shape + `auditor` token; audited + `critic` token), plan-level ledger-shape strictness once frontmatter is `rigor: audited`, and D7 dispatched-task ledger presence on audited multi-agent waves — tightened in place; the epic merge-target check and the intent-scoped Step-5 keys (`refactor`/`tune`) remain **log-only** (append to `.bionic/memory/sdlc-v11-audit.md`, never block) |
| `12` | **Current** | Same universal contract as v11, plus: the scale enum widens to `task`\|`wave`\|`epic`\|`continuous`; `bugfix`\|`spike`\|`incident-response` × `continuous` join the barred cells (SR-1); `scale: continuous` with `rigor:` below `audited` is a new **hard-blocking** fifth floor source; the matrix requirement additionally exempts `scale: continuous`; AND the evidence-gate's version dispatch becomes **fail-loud** (an unrecognized non-empty version now blocks instead of silently passing with zero gating — symmetric with this hook's own always-fail-loud allowlist) | v10 shape table + matrix **unchanged** on wave/epic/task plans (a unified v11/v12 arm — inherited verbatim); adds the **charter contract** for `scale: continuous` — a `## Goals` registry (4-column, `id`/`triple`/`plan`/`status`) addressed via `current: G<n>`, D7 `## Tasks` explicit n/a (§Charter contract (v12)) |

Legacy plans (v1/v2) run grandfathered indefinitely: the governing-skill hook checks only `governing-skill:`, and the evidence gate uses presence-only. v3 plans remain enforced under the prior 5 + 2 contract. v4 through v12 require `model_plan`. v3/v4 plans run the v3 per-step shape table (v4 shares v3's); v5 plans run the v5 shape table; v6 plans run the v6 shape table; v7 plans run the v7 shape table; v8 plans run the v8 shape table; v9 plans run the v9 shape table; v10, v11, and v12 wave/epic plans run the v10 shape table (see [Verification Matrix: a worked example](#verification-matrix-a-worked-example)); v12 `scale: continuous` charters run the [charter contract](#charter-contract-v12) instead. **Never retrofit a newer version's requirements onto an older plan** — an in-flight plan would start blocking mid-wave; this covers every universal-key addition (v7 `bundle-fresh:`, v8 `drive-check:`, v9 `stack-health:`), the v10 Verification Matrix, the v11 axis triple, and the v12 `continuous` scale alike — never retrofit `scale: continuous` onto a v11-or-earlier plan (the enum blocks it). **v≤10 plans keep their `mode:` vocabulary forever** — a `mode:`-declared plan is never retrofitted to the triple, and the governing-skill hook keeps enforcing its `mode:`-gated flag contract unchanged. The one sanctioned mid-life version bump is the reopened-wave upgrade path (v9→v10; see the v10 evidence-shape section above).
...
- **v11** splits the single `mode` axis into the orthogonal `intent · rigor · scale` triple. `mode` conflated three independent questions — what kind of work, how rigorous, how big — so a `spike` couldn't be `audited` and an `autonomous` build couldn't drop to a task-sized lane; the triple lets each vary independently (subject to the floors). The v10 evidence machinery is retained wholesale — the shape table and matrix are unchanged on wave/epic plans — because the axis split is a *classification* change, not an evidence-shape change. v11's new judgment surfaces (task-ledger addressing, epic merge-target, and the intent-scoped `refactor`/`tune` Step-5 keys) shipped **log-only** at first, appending to `.bionic/memory/sdlc-v11-audit.md` and never blocking — the same "kept because it caught something" discipline the ladder runs on. The task-ledger checks have since been tightened **in place, under the same `canonical_sdlc_version: 11`**: ledger-row evidence is now **blocking and rigor-keyed** (§Task-scale ledger enforcement), and an audited multi-agent wave must carry its dispatched-task ledger section at all (D7 presence). The epic merge-target check and the intent-scoped Step-5 keys remain log-only, promoted only if the audit data one day earns it.
- **v12** widens the scale axis with a fourth value, `continuous` — a first-class standing plan kind (the charter), never a bigger epic: heterogeneity distinguishes it, an epic decomposing homogeneously into waves of one intent versus a charter supervising a heterogeneous stream of task/wave/epic sub-goal runs. The wave/epic/task evidence machinery is inherited wholesale from v11 (a unified `gate =11||=12` arm) — v12 adds exactly one new evidence shape, the `## Goals` registry + `current: G<n>` addressing contract for `scale: continuous` (§Charter contract (v12)), plus one hardening that applies to every version: the evidence-gate's version dispatch becomes **fail-loud** (AC-8) — an unrecognized `canonical_sdlc_version` now blocks instead of silently exiting 0 with zero gating, fixing an asymmetry against the governing-skill hook's pre-existing unknown-version block. `scale: continuous` also introduces the ladder's first **hard-blocking** rigor floor (the four v11-and-earlier floor sources stay log-only advisories) — the charter's audited-only footprint is checked at write time, every time, not just on audit.
...
Step 0 is mandatory for new plans (`canonical_sdlc_version: 12`, current). It sets every plan-shaping flag deliberately, with explicit user confirmation, in seven sub-steps:
...
| Step | Gate | Evidence |
|---|---|---|
| 0. Configure | Frontmatter has `canonical_sdlc_version: 12` + the `intent`/`rigor`/`scale` triple + 5 discriminator + 2 opt-in flags + `model_plan`; `## Verification Matrix` derived (or `## Goals` registry for a charter); user confirmed; TaskCreate list created | Confirmation line in `## SDLC State` |
