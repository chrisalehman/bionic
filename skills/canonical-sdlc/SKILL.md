...
7. **Model & dispatch tier** (see §Model & Token Strategy). Default to a **fresh** subagent: mechanical / search / test-writing → fresh `model: sonnet`; implementation slices → fresh `model: sonnet` or `model: opus` per the slice's `complexity:` tag; root-cause debugging and Step 6 review → fresh `model: opus`. Fork only when the unit genuinely needs the inherited conversation/context — never to save effort or to get a cheaper model (§Model & Token Strategy for why forks are expensive on both). Never switch the *main* model to run sub-work.
