...
**Auto-arm ceremony [EXPERIMENTAL].** On plan approval, if the confirmed `watchdog` is effective `on`, the orchestrator arms the goal immediately — calling `sdlc_arm` (lib) with the session's plan path, cwd, session id, and pid. Arm failure never blocks the wave: warn, and record a `## Assumptions` entry naming the failure. **Take-over protocol:** before reopening a session belonging to a machine-driven (`pilot: auto`) goal, declare `pilot: manual` or disarm FIRST — reopening mid-poke without declaring first is the reopen-mid-poke race (§Watchdog and pilot — universal keepalive).
