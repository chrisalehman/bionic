...
| Intent-scoped Step-5 keys (`intent: refactor` / `intent: tune`) | `refactor` additionally carries `behavior-preservation:` (required) and, for migrations/upgrades only, `compat-matrix:`/`revert-plan:` (or `n/a: not a migration`); `tune` additionally carries `baseline:`/`target:`/`re-measure:` (all three required). These are conditional per-intent keys, not members of the universal Step-5 key family. | Log-only (D14, check-ids `refactor-evidence` and `tune-evidence`): each finding appends one line to `.bionic/memory/sdlc-v11-audit.md` and echoes to stderr, then exits 0 — never blocks. Promotion to blocking is a later user decision from the audit data. |
