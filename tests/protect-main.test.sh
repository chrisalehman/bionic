...
# FUNCTION now — `wall_protect_main` in payload/scripts/lib/walls.sh — registered through
...
# REQ-3 N2 (wave-20 T3, D3-9): env's own options used to leave an option as argv[0], so none
# of these was read as a push at all. The argv reader now skips env and its options.
expect_block "REQ-3 env -C <dir>"                 "env -C $ENGAGED_REPO git push origin main"
expect_block "REQ-3 env -C<dir> (value inline)"   "env -C$ENGAGED_REPO git push origin main"
expect_block "REQ-3 env --chdir=<dir>"            "env --chdir=$ENGAGED_REPO git push origin main"
expect_block "REQ-3 env -i"                       "env -i git push origin main"
expect_block "REQ-3 env -u NAME"                  "env -u GIT_ASKPASS git push origin main"
expect_block "REQ-3 env --"                       "env -- git push origin main"
expect_block "REQ-3 /usr/bin/env"                 "/usr/bin/env git push origin main"
expect_block "REQ-3 /usr/bin/env -C <dir>"        "/usr/bin/env -C $ENGAGED_REPO git push origin main"
expect_block "REQ-3 env -S '<string>'"            "env -S 'git push origin main'"
expect_block "REQ-3 env -C <dir> force push"      "env -C $ENGAGED_REPO git push --force origin feature"
...
expect_allow "REQ-3 env -C <dir> of a non-push"    "env -C $ENGAGED_REPO git status"
expect_allow "REQ-3 env -C <dir> push to a topic branch" "env -C $ENGAGED_REPO git push origin topic/main"
expect_allow "REQ-3 echo of an env -C push line"  'echo "env -C /r git push origin main"'
