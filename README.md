...
| **Tier-2 dependencies** | What `/bionic:setup` asks about: the shared basics (git, node, pnpm, gh, jq, rg, uv, docker, aws) and a short list of optional extras, default no (ccstatusline, notebooklm, context7, `@pencil.dev/cli`). Tools a specific workflow needs — the Playwright CLI and browser, chrome-devtools, motion, impeccable — are offered the first time that workflow runs, never up front. The dependency table in [`payload/scripts/lib/deps.sh`](payload/scripts/lib/deps.sh) is the single place each tool's class, consumer, and version constraint are declared, and `/bionic:doctor` reports every row against it |
