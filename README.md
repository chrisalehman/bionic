...
`UserPromptSubmit` hook that re-asserts the CLAUDE.md terseness rules on every turn — and, as of this hook, governs content selection as well as phrasing. CLAUDE.md is loaded once at session start; as context grows and other skills inject competing instructions, the terseness baseline gets diluted. This hook keeps a compact (<1000-char) reminder in the model's attention each turn — lead with the answer, no preamble, drop hedging, don't narrate reasoning — plus a classify-before-asking heuristic (auto-resolve via standing principles, or just act, rather than asking) so the hook governs WHAT gets said and escalated, not only HOW. An explicit EXEMPT clause (code, diffs, commit messages, evidence blocks, the 5-axis review rubric, ADRs, plan/spec frontmatter, security warnings, anything a skill mandates structured output for) keeps terseness from corrupting the structured output that canonical-sdlc and other skills depend on. Disable mid-session with `touch ~/.claude/.bionic-terse-off`; re-enable by removing the flag. Fails silent — a missing `jq` or any error injects nothing rather than breaking the session. Test suite: [`hooks/terseness-reminder.test.sh`](hooks/terseness-reminder.test.sh).
