...
# mid-run (tests/run.sh:83-89, W7 assumption A4.2). So the memory term is measured and
...
MEM_RESERVE_GB=2       # datum: 2026-08 measurement, tests/run.sh:83-89 — the 8 GB machine
...
MEM_PER_SUITE_GB=1.2   # datum: 8 GB, 7 concurrent suites, ~188 MB free, kernel SIGKILL,
                       # tests/run.sh:83-89 — (8 − 2) GB across 7 suites is ~0.86 GB each
...
TEST_JOBS_MIN=4        # datum: tests/run.sh:83-89 — 4 was "the width with headroom" on the
...
EMERGENCY_FREE_MB=256  # datum: kill at ~188 MB (tests/run.sh:83-89) — the floor sits just
