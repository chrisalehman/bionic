...
# v3 through v7 are all enforced. v4 added a required `model_plan` (the
...
# hook — no flag change here). v4/v5/v6/v7 share v4's flag
# contract unchanged. v3 plans keep the prior contract so in-flight plans
# authored before v4 are not retroactively broken.
if [ "$SDLC_VERSION" != "3" ] && [ "$SDLC_VERSION" != "4" ] && [ "$SDLC_VERSION" != "5" ] && [ "$SDLC_VERSION" != "6" ] && [ "$SDLC_VERSION" != "7" ]; then
  echo "BLOCKED: canonical-sdlc artifact '$BASENAME' has unsupported canonical_sdlc_version: '$SDLC_VERSION'." >&2
  echo "Path: $FILE_PATH" >&2
  echo "Fix: set 'canonical_sdlc_version: 7' (current), '6'/'5'/'4'/'3' (prior, still enforced), or '1'/'2' for legacy plans." >&2
  exit 2
fi
...
if [ "$SDLC_VERSION" = "4" ] || [ "$SDLC_VERSION" = "5" ] || [ "$SDLC_VERSION" = "6" ] || [ "$SDLC_VERSION" = "7" ]; then
