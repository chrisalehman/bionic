...
# v3, v4, v5, and v6 are all enforced. v4 added a required `model_plan` (the
# Step-0 model-tier decision); v5 collapses the step set (gate model); v6
# removes the external-review step (0–9 shape). v4/v5/v6 share v4's flag
# contract unchanged. v3 plans keep the prior contract so in-flight plans
# authored before v4 are not retroactively broken.
if [ "$SDLC_VERSION" != "3" ] && [ "$SDLC_VERSION" != "4" ] && [ "$SDLC_VERSION" != "5" ] && [ "$SDLC_VERSION" != "6" ]; then
  echo "BLOCKED: canonical-sdlc artifact '$BASENAME' has unsupported canonical_sdlc_version: '$SDLC_VERSION'." >&2
  echo "Path: $FILE_PATH" >&2
  echo "Fix: set 'canonical_sdlc_version: 6' (current), '5'/'4'/'3' (prior, still enforced), or '1'/'2' for legacy plans." >&2
  exit 2
fi
...
# v4, v5, and v6 additionally require the confirmed `model_plan` field (the
...
if [ "$SDLC_VERSION" = "4" ] || [ "$SDLC_VERSION" = "5" ] || [ "$SDLC_VERSION" = "6" ]; then
